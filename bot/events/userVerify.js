...
module.exports = async function(discordID, vlcName) {
  ...
  */
  const verifyLogsChannel = await discordClient.channels.fetch(globals.verifyLogsChannelID);
  const commLogsChannel = await discordClient.channels.fetch(globals.commLogsChannelID); 
  ...
  discordClient.guilds.cache.each(async guild => {
    if (guild.members.cache.has(discordID)) {
      ...
      try {
        ...
        globals.error(`Unable to add verified role to <@${discordID}> (\`${discordID}\`) in **${guild.name}**.\n\`\`\`\n${error}\n\`\`\``);
      }
    }
  });
    ...
    commLogsChannel.send(`✅ <@${discordID}> (\`${discordID}\`) has successfully verified as **${vlcName}**.`);
